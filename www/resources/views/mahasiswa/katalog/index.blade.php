    ...
    <div class="catalog-search-wrap">
        <div class="catalog-top-left">
            <div class="catalog-search-box">
                ...
                @if(request('search'))
                <a href="{{ route('katalog.index') }}" style="color:#9CA3AF;text-decoration:none;padding:4px;display:flex">&times;</a>
                @endif
            </div>
        ...
        <div class="catalog-hero-right">
            <a href="{{ route('katalog.index') }}" class="catalog-filter-box {{ !request('status_alat') ? 'active' : '' }}">
                <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16m-7 6h7"/>
                </svg>
                Semua
            </a>
            <a href="{{ route('katalog.index', array_merge(request()->except('status_alat', 'page'), ['status_alat' => 'TERSEDIA'])) }}" class="catalog-filter-box {{ request('status_alat') === 'TERSEDIA' ? 'active' : '' }}">
                <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="catalog-card">
        ...
        <div class="catalog-card-body">
            ...
            <div class="catalog-card-actions">
                <a href="{{ route('katalog.show', $tool->id_alat) }}" class="btn-detail">
                    <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.querySelectorAll('.catalog-filter-box').forEach(function(el) {
    el.addEventListener('click', function(e) {
        if (this.getAttribute('href') === '{{ route('katalog.index') }}' && ({{ request('status_alat') ? 'true' : 'false' }} || {{ request('kategori') ? 'true' : 'false' }})) {
