...
<a href="{{ route('admin.peminjaman.index', ['tab' => request('back_tab', 'semua')]) }}" class="btn btn-sm btn-outline">&larr; Kembali</a>
@endsection
...
<div class="card" style="margin-bottom:16px">
    ...
    <div style="display:flex;gap:8px">
        <form method="POST" action="{{ route('admin.peminjaman.approve', $borowing->id_borrowing) }}" style="display:inline" onsubmit="return confirmAction('Setujui peminjaman ini?')">
            @csrf
    ...
    @endif
    @if($borowing->status === 'DIPINJAM')
    <hr class="divider">
    <a href="{{ route('admin.peminjaman.aktif') }}" class="btn" style="background:#8B5CF6">Catat Pengembalian</a>
    @endif
</div>
