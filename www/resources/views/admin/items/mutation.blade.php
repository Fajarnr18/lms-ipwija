...
<div class="card" style="margin-bottom:16px">
    <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
        <div style="flex:1;min-width:200px">
            ...
            <div style="font-size:12px;color:#6B7280;margin-top:2px">Kode: {{ $item->kode_barang }}</div>
        </div>
        <div style="text-align:right">
            ...
            <div style="font-size:22px;font-weight:700;color:#1E4FD8">{{ $item->stok }} <span style="font-size:13px;font-weight:500;color:#6B7280">{{ $item->satuan }}</span></div>
        </div>
    </div>
</div>

<div class="card" style="margin-bottom:16px">
    <form method="POST" action="{{ route('admin.items.mutation-store', $item->id_barang) }}">
        @csrf
    ...
    <div style="overflow-x:auto">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="7">
                        <div class="empty-state">Belum ada riwayat mutasi.</div>
                    </td>
                </tr>
                @endforelse
            </tbody>
