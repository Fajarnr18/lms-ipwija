...
@section('content')
<form id="form-alat" method="POST" action="{{ route('admin.alat.update', $tool->id_alat) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
        ...
        <div class="card">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
                <div class="form-group" style="margin-bottom:0">
                    ...
                    <input type="text" name="kode_alat" value="{{ old('kode_alat', $tool->kode_alat) }}" required placeholder="Contoh: AL-001">
                    @error('kode_alat')<div class="error-text">{{ $message }}</div>@enderror
                </div>
                <div class="form-group" style="margin-bottom:0">
                    <label>Kategori</label>
                    <input type="text" name="kategori" value="{{ old('kategori', $tool->kategori) }}" required placeholder="Elektronik, Kimia, dll">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama_alat" value="{{ old('nama_alat', $tool->nama_alat) }}" required>
                @error('nama_alat')<div class="error-text">{{ $message }}</div>@enderror
            </div>

            <div class="form-group">
                <label>Deskripsi Alat</label>
                <textarea name="deskripsi" rows="3" required>{{ old('deskripsi', $tool->deskripsi) }}</textarea>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px">
                <div class="form-group" style="margin-bottom:0">
                    ...
                    <input type="text" name="lokasi" value="{{ old('lokasi', $tool->lokasi) }}" required placeholder="Rak A1">
            ...
            <div class="card">
                ...
                <div class="form-group">
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:6px">
                        <label style="flex:1;display:flex;align-items:center;gap:8px;padding:10px 12px;border:1.5px solid #E5E7EB;border-radius:6px;cursor:pointer;transition:all .15s;font-size:13px;font-weight:500">
                            <input type="radio" name="status_alat" value="TERSEDIA" {{ old('status_alat', $tool->status_alat) === 'TERSEDIA' ? 'checked' : '' }} style="width:auto">
                            Tersedia
                        </label>
                        <label style="flex:1;display:flex;align-items:center;gap:8px;padding:10px 12px;border:1.5px solid #E5E7EB;border-radius:6px;cursor:pointer;transition:all .15s;font-size:13px;font-weight:500">
                ...
                <div class="form-group" style="margin-bottom:0">
                    ...
                    <select name="kondisi_fisik">
                        ...
                        <option value="Baik" @selected(old('kondisi_fisik', $tool->kondisi_fisik)==='Baik')>Baik</option>
                        <option value="Rusak Ringan" @selected(old('kondisi_fisik', $tool->kondisi_fisik)==='Rusak Ringan')>Rusak Ringan</option>
                        <option value="Rusak Berat" @selected(old('kondisi_fisik', $tool->kondisi_fisik)==='Rusak Berat')>Rusak Berat</option>
                    </select>
